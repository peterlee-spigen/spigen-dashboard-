...
const NATIONAL_COUNTRIES = ["UK", "DE", "FR", "IT", "ES"];
...
export default function ReportsPage() {
  ...
  const [selectedKey, setSelectedKey] = useState<string>("UK"); // country or category
  const [data, setData] = useState<(CountryReport | CategoryReport)[]>([]);
  const [loading, setLoading] = useState(true);
  const [fetchedAt, setFetchedAt] = useState<Date | null>(null);

  const loadData = useCallback((tab: ReportTabId) => {
    setLoading(true);
    fetch(`/api/sheets/${tab}`)
      ...
      .then((d) => {
        ...
        const first = d[0];
        if (first) {
          setSelectedKey("country" in first ? first.country : first.category);
        }
        setLoading(false);
      })
  ...
  const timeAgo = fetchedAt
    ? `${Math.round((Date.now() - fetchedAt.getTime()) / 1000)}초 전 로드`
    : null;
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-neutral-500 bg-neutral-100 dark:bg-neutral-800 px-2 py-1 rounded">
            📊 Google Sheets 연동 · 최대 60초 캐시
          </span>
          <button
            onClick={() => loadData(reportTab)}
          ...
          {!report ? (
            ...
            <>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  { label: "광고 매출",    value: fmt(latest?.sales ?? null, curr),  d: delta(latest?.sales ?? null,  prev?.sales ?? null) },
                  { label: "광고비 Spend", value: fmt(latest?.spend ?? null, curr),  d: delta(latest?.spend ?? null, prev?.spend ?? null) },
                  ...
                  >
                    ...
                    <p className="text-xl font-bold text-neutral-900 dark:text-neutral-50 truncate">
                      {value}
                    </p>
                    <DeltaBadge d={d} />
                  </div>
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-neutral-50 dark:bg-neutral-800 text-neutral-500 text-xs">
                      <tr>
                        {["월", "노출", "클릭", "CTR", "광고비", "ACOS", "주문", "매출"].map((h) => (
                          <th key={h} className="px-4 py-2 text-right first:text-left whitespace-nowrap">
                            {h}
                          </th>
                        ))}
                      </tr>
                    ...
                    <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800">
                      {report.months.slice(0, 16).map((month, i) => (
                        <tr
                          key={month}
                          className={i === 0 ? "bg-blue-50/50 dark:bg-blue-950/20 font-medium" : ""}
                        >
